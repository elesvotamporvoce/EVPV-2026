...
export default async function PersonPage({
  ...
  const { data: candRow } = await supabase
    ...
    .select("cargo, uf, situacao, patrimonio_total, sexo")
    .eq("person_id", id)
    .maybeSingle();
...
<Voltar fallback="/pessoas" />
      ...
      <div className="relative rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
        {cand && (
          ...
          >
            <SeloCandidato feminino={cand.sexo === "F"} />
          </Link>
        )}
        {/* Foto a esquerda e texto a direita, e o conjunto centralizado na
            pagina. O cabecalho e sticky durante a rolagem, entao ele e o mais
            baixo possivel: so nome, cargo, partido/estado e mandato. */}
        <div className="flex items-center justify-center gap-4">
          <span className="h-20 w-20 shrink-0 overflow-hidden rounded-full bg-slate-100 sm:h-24 sm:w-24">
            {dir.photo_url ? (
          ...
          <div className="min-w-0">
            ...
            <p className="mt-0.5 text-slate-500">{CARGO_LABEL[dir.house]}</p>
